...
//    std::cout << m_CentralMoments << std::endl;
    ...
    for( int i=0; i<ImageDimension; i++ )
      {
      minPrincipalMoment = std::min( pm(i,i), minPrincipalMoment );
      maxPrincipalMoment = std::max( pm(i,i), maxPrincipalMoment );
      }
  
    compactness = 1.0;
    if( maxPrincipalMoment != 0 )
      {
      compactness = minPrincipalMoment / maxPrincipalMoment;
      }
    }
