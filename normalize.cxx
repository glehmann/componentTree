...
int main(int argc, char * argv[])
{
  ...
  typedef itk::NormalizeComponentTreeFilter< TreeType > NormalizeType;
  NormalizeType::Pointer norm = NormalizeType::New();
  norm->SetInput( filter->GetOutput() );
  itk::SimpleFilterWatcher watcher(norm, "filter");

  typedef itk::ComponentTreeAttributeToImageFilter< TreeType, IType2 > T2IType;
  T2IType::Pointer filter2 = T2IType::New();
  filter2->SetInput( norm->GetOutput() );
